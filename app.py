...
st.sidebar.markdown("---")
industry_options = list(INDUSTRY_TEMPLATES.keys())
if "industry_template" not in st.session_state:
    st.session_state.industry_template = "Banking"
if st.session_state.industry_template not in INDUSTRY_TEMPLATES:
    st.session_state.industry_template = "Banking"

industry = st.sidebar.selectbox(
    "Industry Template",
    industry_options,
    index=industry_options.index(st.session_state.industry_template),
    key="industry_template",
)

industry_sops = INDUSTRY_TEMPLATES[industry]

if "selected_template" not in st.session_state:
    st.session_state.selected_template = industry_sops[0]
if st.session_state.selected_template not in industry_sops:
    st.session_state.selected_template = industry_sops[0]

st.sidebar.markdown("### Available Templates")
selected_template = st.sidebar.radio(
    "Available Templates",
    industry_sops,
    index=industry_sops.index(st.session_state.selected_template),
    key="selected_template",
    label_visibility="collapsed",
)

# Navigation radio placed after templates
